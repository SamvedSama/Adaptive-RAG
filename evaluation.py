...
def evaluate_config(
    ...
    if predictions:
        try:
            ...
            logger.warning("bert_score not installed. Please install it.")

    result: dict[str, Any] = {
        "n_queries":           n,
        "exact_match":         round(sum(em_list)      / n, 4) if n else 0.0,
        "token_f1":            round(sum(f1_list)      / n, 4) if n else 0.0,
        "bert_f1":             round(bert_f1_score, 4) if n else 0.0,
...
def evaluate(
    ...
    # ── Console summary ───────────────────────────────────────────────
    print("\n" + "─" * 70)
    print(f"  {'Config':<28} {'EM':>6} {'F1':>6} {'BertF1':>6} {'Succ':>6} {'Faith':>7} {'Rel':>7} {'CtxR':>7}")
    print("─" * 70)
